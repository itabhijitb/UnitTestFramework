...
namespace PTable
{
	...
	struct TITLE {};
	template<Allignment>
	std::string Allign(std::string st, size_t size)
	{
		if (size < st.size())
	...
	template<>
	std::string Allign<Center>(std::string st, size_t size)
	{
		std::stringstream ss_cell;
		if (size < st.size())
	...
	template<>
	std::string Allign<Right>(std::string st, size_t size)
	{
		if (size < st.size())
			st.erase(st.begin() + st.size() - size, st.end());
		std::stringstream ss_cell;
	...
	public:
		...
		friend std::ostream& operator<<(std::ostream& os, const Cell& cell)
		{
			...
			{
			case Left: 
				st   << Allign<Left>(cell.m_value, cell.m_size); 
				break;
			case Right: 
				st  << Allign<Right>(cell.m_value, cell.m_size); 
				break;
			case Center: 
				st << Allign<Center>(cell.m_value, cell.m_size); 
				break;
			default: break;
			}
			st << cell.m_col_sep;
